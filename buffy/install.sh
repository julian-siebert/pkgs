...
#   BUFFY_INSTALL_DIR  Install location (default: $HOME/.local/bin)
#   BUFFY_NO_MODIFY_PATH  Set to any value to skip PATH modification
...
BASE_URL="${BUFFY_BASE_URL:-https://pkgs.julian-siebert.de/buffy}"
VERSION="${BUFFY_VERSION:-latest}"
...
main() {
    info "Buffy installer"
    check_prerequisites

    target=$(detect_target)
    info "Detected platform: ${BOLD}$target${RESET}"

    tmp=$(mktemp -d 2>/dev/null || mktemp -d -t buffy-install)
    ...
    info "Installing buffy ${BOLD}v$resolved_version${RESET}"
    ...
    done_ "buffy v$resolved_version installed to $INSTALL_DIR/$binary_name"

    if ! is_in_path "$INSTALL_DIR" && [ -z "${BUFFY_NO_MODIFY_PATH:-}" ]; then
        suggest_path_setup
    else
        echo
        echo "Run ${BOLD}buffy --help${RESET} to get started."
    fi
}
